...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { data, error } = await supabase.from("dispatch_offers").select("id, order_id, status, required_class, distance_mi, expires_at, orders(order_number, pickup_city, dropoff_city, pickup_address, dropoff_address, service_type)").eq("driver_id", driverId).eq("status", "offered").order("created_at", { ascending: false }).limit(1);
